 ...
 * @author StackPointer
 */
@ManagedBean(name = "nivelGeneracion")
@SessionScoped
public class NivelGeneracionBean implements Serializable {

    @EJB
    private EstructuraDPAFacadeLocal facEstDPA;
    @ManagedProperty(value = "#{usuarioSesion}")
    private UsuarioSesionBean usuarioSesion;
    @ManagedProperty(value = "#{indicador}")
    ...
    public void generarIndicadores() {
        ...
        registrar.setNombreMetodo(BitacoraTransaccion.GENERAR_INDICADORES.name());
        registrar.setUsuario(usuarioSesion.getUsuario().getUsername());
        registrar.start();
        indicador.generarIndicadores(nivelSeleccionado);
        init();
    }
