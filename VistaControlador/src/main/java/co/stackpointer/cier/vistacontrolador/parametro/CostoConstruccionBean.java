 ...
 * @author user
 */
@ManagedBean(name = "costoConstruccionBean")
@ViewScoped
public class CostoConstruccionBean implements Serializable {

    @EJB
    private ParametrosFacadeLocal facParam;
    private List<CostoConstruccion> lista;
    ...
    public void consultar() {
        try {
            ...
            registrar.setNombreMetodo(BitacoraTransaccion.CONSULTA_COSTO_CONSTRUCCION.name());
            registrar.setUsuario(usuarioSesion.getUsuario().getUsername());
            registrar.start();
        }
    }

    public List<CostoConstruccion> getLista() {
        return lista;
    }

    public void actualizar(RowEditEvent event) {
        try {
            facParam.modificarCostoConstruccion((CostoConstruccion) event.getObject());
            actualizarLista();;
        } catch (Exception e) {
            ...
            RegistrarAuditoria registrar = new RegistrarAuditoria();
            registrar.setFecha(new Date());
            registrar.setNombreMetodo(BitacoraTransaccion.EDITAR_COSTO_CONSTRUCCION.name());
            registrar.setUsuario(usuarioSesion.getUsuario().getUsername());
            registrar.start();
        }
    }
    ...
    private void actualizarLista() {
        lista = facParam.consultarCostoConstrucciones();
    }
